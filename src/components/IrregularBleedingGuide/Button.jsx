...
import { Info, TriangleAlert, Stethoscope, FileText, TestTube, Pill } from "lucide-react";
...
const IrregularBleedingGuideButtons = ({ setShowCard, activeButton, setActiveButton }) => {
  ...
  return (
    <div
      className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-6 
      ...
      {buttons.map((button, index) => (
        <button
          ...
          className={`flex flex-col items-center justify-center text-center 
            ...
            text-black bg-light-pink-1 hover:bg-primary-pink/30 transition-all
            ${activeButton === index ? "bg-light-pink-2 text-black shadow-md" : ""}`}
        >
          {button.icon}
