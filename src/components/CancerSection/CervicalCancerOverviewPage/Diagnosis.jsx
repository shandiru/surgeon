  ...
  return (
    <section className="py-12" data-aos="fade-up">
      <div className="max-w-6xl mx-auto px-4 md:px-6 lg:px-8">
        <div className="bg-[#fff5f8] text-primary-pink flex flex-col gap-6 rounded-xl border border-[#f5a9bd] border-l-4 border-l-[#FF4B8B] shadow-sm py-6">
          {/* Header */}
              ...
              <div className="grid gap-4">
                {methods.map((method, index) => (
                  <div
                    ...
                    data-aos="fade-left"
                  >
                    <div className="w-8 h-8 flex items-center justify-center rounded-full bg-[#FFC5D3] text-primary-pink text-sm font-bold flex-shrink-0">
                      {index + 1}
                    </div>
                    <div>
                      <h4 className="font-medium text-primary-pink">{method.title}</h4>
                      <p className="text-sm text-[#7a2f4f] mt-1">– {method.description}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div data-aos="fade-up">
              ...
              <div className="flex flex-wrap gap-2">
                {imagingTests.map((test, idx) => (
                  <span
                    key={idx}
                    className="text-sm border border-[#f5a9bd] bg-[#FFC5D3] text-primary-pink px-3 py-1 rounded-md font-medium"
                  >
                    {test}
                  </span>
                ))}
